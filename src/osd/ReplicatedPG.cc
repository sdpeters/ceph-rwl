...
bool ReplicatedPG::_check_new_object(OpContext *ctx)
{
...
int ReplicatedPG::do_osd_ops(OpContext *ctx, vector<OSDOp>& ops)
{
  ...
  for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p, ctx->current_osd_subop_num++) {
   ...
   case CEPH_OSD_OP_NOTIFY:
	...
	if (_check_new_object(ctx)) {
	  t->touch(obs.oi.soid);
#warning why no mod_desc create here?
	  //ctx->mod_desc.create();
	}
	...
	if (_check_new_object(ctx)) {
	  t->touch(soid);
#warning why no mod_desc create?
	  //ctx->mod_desc.create();
	} else {

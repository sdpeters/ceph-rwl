                           ...
                           bool intra_region)
{
  ...
  case RGWRados::ATTRSMOD_MERGE:
    for (map<string, bufferlist>::iterator it = attrs.begin(); it != attrs.end(); ++it) {
      /* preserve source acls if it's intra-region operation */
      if (it->first != RGW_ATTR_ACL || !intra_region) {
               ...
               void *progress_data)
{
  ...
  set_copy_attrs(src_attrs, attrs, attrs_mod, !source_zone.empty());
               ...
               void *progress_data)
{
  ...
  set_copy_attrs(src_attrs, attrs, attrs_mod, false);

...
#define OID_EVENT_TRACE(oid, event) \
  EventTrace::trace_oid_event(oid, event, "", __func__)
#define OID_EVENT_TRACE_WITH_MSG(msg, event, incl_oid) \
  EventTrace::trace_oid_event(msg, event,  __func__,  incl_oid)
#define OID_ELAPSED(oid, elapsed, event) \
  EventTrace::trace_oid_elapsed(oid, event, "", elapsed, __func__)
#define OID_ELAPSED_WITH_MSG(m, elapsed, event, incl_oid) \
  EventTrace::trace_oid_elapsed(m, event, elapsed,__func__,  incl_oid)
#define FUNCTRACE() EventTrace _t1(g_ceph_context,  __func__)
...
public:
  ...
  EventTrace(CephContext *_ctx, const char *_func);
  ~EventTrace();
  void log_event_latency(const char *tag);

  static void trace_oid_event(const char *oid, const char *event, const char *context,
    const char *func);
  static void trace_oid_event(const Message *m, const char *event, const char *func, bool incl_oid);

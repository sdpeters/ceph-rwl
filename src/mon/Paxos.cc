...
void Paxos::trim()
{
  ...
  MonitorDBStore::Transaction *t = get_pending_transaction();
...
MonitorDBStore::Transaction *Paxos::get_pending_transaction()
{
  assert(mon->is_leader());
  if (!pending_proposal) {
    pending_proposal = new MonitorDBStore::Transaction;
